...
支持多台电脑之间的日级数据同步。启动时执行一次，之后每 30 分钟上传本设备数据并拉取所有设备数据合并到本地。

### 同步范围

- `tag_time_records`：按日期+标签汇总（如 2026-08-19 / Work / 14400s）
- `time_records`：按日期+应用+项目汇总
- `ai_token_daily`：按日期+模型/来源汇总 AI Token
- `tool_call_daily`：按日期+类别+名称汇总 MCP/Skill 调用
- **不同步** `time_segments`（秒级事实表，数据量太大）

### 同步策略

- **时机**：启动时及之后每 30 分钟，同步 `date <= today` 的数据
